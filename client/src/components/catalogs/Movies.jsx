...
export default function Movies() {
    ...
    <div className="bg-gray-900 text-yellow-400 min-h-screen py-8 px-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {movies.map((movie) => (
          <div key={movie._id} className="bg-gray-800 rounded-lg overflow-hidden shadow-xl transform transition-all duration-300 hover:scale-105 hover:shadow-2xl relative">
            <div className="relative group">
              <img src={movie.poster} alt={movie.title} className="w-full items-center h-90 object-cover rounded-t-lg" />
              {/* Hover Overlay */}
            ...
            <div className="p-4">
              ...
              <p className="text-gray-400 mt-2">Release Date: {movie.year}</p>
            </div>
          </div>
        ))}
      </div>
